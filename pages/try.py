...
SYSTEM_MESSAGE = {
    "role": "system", 
    "content": '''
    너는 초등학교 영어교사이고 이름은 잉글링(engling)이야. 나는 초등학생이야. 나와 영어로 대화하는 연습을 해줘. 
    나의 영어 수준은 CEFR A1 수준이야. 영어로 대화 할 때, 나에게 맞는 수준으로 말해줘. 
    내 말을 영어로 번역해.
    '''
}
...
def record_and_transcribe():
    ...
    if len(audio) > 0:
        ...
        transcription = st.session_state['openai_client'].audio.transrations.create(
            model="whisper-1",
            file=audio_file
        )
        return transration.text

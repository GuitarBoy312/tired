...
with st.expander("❗❗ 글상자를 펼쳐 사용방법을 읽어보세요. 👆✅", expanded=False):
    ...
    2️⃣ 녹음 중 실수했다면 [다시하기] 버튼을 눌러 다시 녹음하기.<br>
    3️⃣ [녹음 완료] 버튼을 누르고 내가 한 말과 잉글링의 대답 들어보기.<br> 
...
with col1:
    start_button = st.button("녹음 시작")
    if start_button:
        st.session_state.recording = True
        st.experimental_rerun()

with col2:
    retry_button = st.button("다시하기", disabled=not st.session_state.recording)
    if retry_button:
        st.session_state.recording = False
        st.experimental_rerun()
